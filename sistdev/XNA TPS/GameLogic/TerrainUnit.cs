    ...
    public class TerrainUnit : DrawableGameComponent
    {
        ...
        public void Jump(float jumpHeight)
        {
            if (isOnTerrain)
            {
                ...
                camera.ChaseSpeed *= 5.0f;
                speed *= 1.5f;
                adjustJumpChanges = true;

                gravityVelocity = (float)GRAVITY_ACCELERATION * jumpHeight * 0.11f;
                isOnTerrain = false;
            }
        }

        public void MiniJump(float jumpHeight)
        {
            if (isOnTerrain)
            {
                (cameraManager.ActiveCamera as ThirdPersonCamera).NeedFolow = false;
                speed *= 1.5f;
                adjustJumpChanges = true;
